...
def build_pipeline_config(neurons, grid_size: tuple[int, int]) -> PipelineConfig:
    ...
    if len(dnp20_ids) != 2:
        raise RuntimeError(f"expected exactly 2 DNp20 neurons (L/R), found {len(dnp20_ids)}")

    neurons_indexed = neurons if neurons.index.name == "bodyId" else neurons.set_index("bodyId")
    instances = neurons_indexed.loc[dnp20_ids, "instance"] if "instance" in neurons_indexed.columns else None
    if instances is not None:
        left_id = next(bid for bid in dnp20_ids if str(instances.get(bid, "")).endswith("_L"))
        right_id = next(bid for bid in dnp20_ids if str(instances.get(bid, "")).endswith("_R"))
    else:
...
def build_graph_and_simulator(export_dir: Path, leak: float, steps: int) -> tuple[ConnectomeGraph, ConnectomeSimulator]:
    neurons, roi_conn = load_connectome_export(export_dir)
    graph = build_connectome_graph(neurons, roi_conn)
    simulator = ConnectomeSimulator(graph.weight_matrix, graph.id_to_index, leak=leak, steps=steps)
...
def run(
    ...
    if launch_dashboard:
        ...
        viz_subset = pick_visualization_subset(
            graph, config.r1_r6_ids, config.r8_ids,
            [config.dnp20_left_id, config.dnp20_right_id], config.dnpe017_ids,
        )
        capture_body_ids = [n.body_id for n in viz_subset.nodes]
        dashboard_root = root if root.is_dir() else root.parent
        dashboard = DashboardServer(event_bus, root=dashboard_root, viz_subset=viz_subset, port=port)
        dashboard.start(open_browser=True)
        print(f"Dashboard: {dashboard.url} ({len(viz_subset.nodes)} real neurons, {len(viz_subset.edges)} real edges)")
        time.sleep(0.3)

    print(f"Processing {len(pngs)} images (connectome simulation is expensive per image)...")
    event_bus.publish(ResultEvent(type="started", total_images=len(pngs)))

    tally = {"pass": 0, "fail": 0, "flag": 0}
    rows = []
    for i, path in enumerate(pngs, 1):
        result = run_pipeline(path, simulator, config, thresholds, capture_body_ids=capture_body_ids)
        ...
        rows.append(
            {
                ...
                "confidence_signal": result.confidence_signal if result.confidence_signal is not None else "",
                "verdict": result.verdict,
                "calibrated": result.calibrated,
                "duration_sec": result.duration_sec,
            }
        )
        print(f"  [{i}/{len(pngs)}] {path.name}: {result.verdict} "
        ...
        event_bus.publish(
            ResultEvent(
                ...
                processed_images=i,
                tally=dict(tally),
                step_activity=result.step_activity.tolist() if result.step_activity is not None else None,
            )
        )
    ...
    if dashboard is not None:
        print("Dashboard still running. Press Enter to exit.")
        try:
            input()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("path", type=Path, help="A PNG file or a directory to scan recursively")
    parser.add_argument(
        "--connectome-export", type=Path,
